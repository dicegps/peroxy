    ...
    , ui(new Ui::MainWindow)
{
    ...
    filePathGlobal = "/home/" + userNameGlobal +"/PeroxyOutput/";
    ...
    //make the necessary shell file that initializes the bash environment (.bash_profile)
    makedir(filePathGlobal);
    makeFile("runner.sh", "#!/bin/bash\n\n. ~/.bash_profile\n/home/share/slurm/19.05.3/bin/sbatch g09.sub");
    sendCmd("chmod +x " +filePathGlobal +"runner.sh");
}
...
int MainWindow::makedir(QString dirPath)
{
...
int MainWindow::calcMolecMultiplicity()
{
    int molecMultiplicity = (ui->atom1Multiplicity->value()+ui->atom2Multiplicity->value())-3;

    if (molecMultiplicity == 0)
        return 1;

    if (molecMultiplicity % 2)  //mult is odd integer
    {
        return 1;
    }
    else                    //mult is even integer
        return 2;
}
...
int MainWindow::makeFile(QString fName, QString fContent)
{
    ...
    QFile file("/home/"+userNameGlobal+"/PeroxyOutput/" + fName);
...
QString getInfoOptFreq(QString filePath, QString fileName, QString wildcard)
{
    ...
    if(myFile.exists() && myFile.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        do {
            line = in.readLine();
            if (line.contains(wildCard2, Qt::CaseSensitive)) {
                found++;
                lineCount = 0;
            }
            if (found == 1)
            {
                ...
                else
                {
                    ...
                    return buf1;   */
                    //found = 2;
                    wildCard2 = "Rotational constant (GHZ):";
                }
            }
            if (found == 2)
            {
                ...
                else
                {
                    ...
                    return buf1;
                    */
                    wildCard2 = "Stationary point found";
                }
            }
            if (found == 3)
            {
                if (lineCount == 7)
                {
                    buf1 = buf1 + "\n------------------\nOptimized Bond Length: " + line.mid(32,8) + "Å";
                    myFile.close();
                    return buf1;
                }
                else
...
QString MainWindow::sendCmd(QString myCmd)
{
    ...
    QProcess *proc = new QProcess();
    proc->setWorkingDirectory("/home/"+userNameGlobal+"/PeroxyOutput/");
...
QString sbatch(QString workPath)
{
    ...
    //starts a shell script that initializes the bash shell's profile (thus setting environment variables for slurm), then executes sbatch g09.sub

    proc->start("runner.sh");
    proc->waitForFinished();
...
{
    ui->textEdit1->setText(sbatch("/home/"+userNameGlobal+"/PeroxyOutput"));
}
...
void MainWindow::performOptFreq(QString filePath, QString comFileName, QString comDescriptor, QString radialDistance, QString userName, QString atom1, QString atom2)
{
    ...
    sbatch("/home/"+userNameGlobal+"/PeroxyOutput");
...
void MainWindow::on_buttonStart_clicked()
{
    ...
    //molecularMultiplicityGlobal = "3";
    int molecBoxValue = ui->molecularMultiplicityBox->value();
    molecularMultiplicityGlobal = QString::number(molecBoxValue);
    ...
    QString filePath = "/home/"+userNameGlobal+"/PeroxyOutput/";
    ...
    {
        if ((calcMolecMultiplicity() % 2) == (molecBoxValue % 2))   //are the calculated multiplicity and user-enetered multiplicities in accordance for a given # of  electrons?
        {
            //Main Section Here:
        ...
        {
            QMessageBox Msgbox;
            Msgbox.setText("Invalid Multiplicity.  Check your multiplicity and try again.");
            Msgbox.exec();
        }
...
void MainWindow::on_atom1Edit_textEdited(const QString &arg1)
{
    ...
    if (possibleElements.contains("," + tempText + "-"))
    {
        ...
        ui->molecularMultiplicityBox->setValue(calcMolecMultiplicity());
...
{
    int readyOptFreq, readySP, readySingle, readyScan = 0;

    if (ui->checkBoxOptFreq->isChecked())
    {
        if (ui->progressBarOptFreq->value() == 100)
            readyOptFreq = 1;
    }
    else
        readyOptFreq = 1;

    if (ui->checkBoxSP->isChecked())
    {
        if (ui->progressBarSP->value() == 100)
            readySP = 1;
    }
    else
        readySP = 1;
    ...
    if (ui->checkBoxIndividual->isChecked())
    {
        if (ui->progressBarSingle->value() == 100)
            readySingle = 1;
    }
    else
        readySingle = 1;
    ...
    if (ui->checkBoxPES->isChecked())
    {
        if (ui->progressBarScan->value() == 100)
            readyScan = 1;
    }
    else
        readyScan = 1;
    ...
    if ((readyOptFreq + readySP + readySingle + readyScan) == 4)
    {
...
{
    ui->spinBoxInitial->setValue(0.5);
    ui->spinBoxSize->setValue(0.1);
    ui->spinBoxSteps->setValue(17);

}

void MainWindow::on_molecularMultiplicityBox_textChanged(const QString &arg1)
{

}

void MainWindow::on_molecularMultiplicityBox_valueChanged(int arg1)
{
    if ((calcMolecMultiplicity() % 2) == (arg1 % 2))
        ui->molecularMultiplicityBox->setStyleSheet("background-color: rgb(255, 255, 255);");
    else
        ui->molecularMultiplicityBox->setStyleSheet("background-color: rgb(255, 0, 0);");
}
